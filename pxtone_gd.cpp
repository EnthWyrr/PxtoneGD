...
// I/O Callbacks
 ...
 static bool pxtn_seek(void* user, int mode, int size)
 {
    ...
    int file_size = file_to_seek_from->get_length();
 	switch (mode)
 	{
 		case 0:	// SEEK_SET
...
PxtoneGD::PxtoneGD() {
    ...
    AudioServer::get_singleton()->connect("bus_layout_changed", callable_mp(this, &PxtoneGD::_bus_layout_changed));
}
...
PxtoneGD::~PxtoneGD() {
 ...
 Error PxtoneGD::load_tune(const String file_path) {
 	...
 	Ref<FileAccess> tune_file = FileAccess::open(file_path, FileAccess::READ, &load_error);
 	if (load_error != OK)
 	{
 		goto finish;
 	}
 	...
 	{
 		pxtn_err = pxtn->read(tune_file.ptr());
 		if (pxtn_err != pxtnOK)
 		{
 			load_error = FAILED;
 ...
 void PxtoneGD::_bus_layout_changed()
 {
 ...
 void PxtoneGD::_bind_methods() {
 	...
 	ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "volume", PROPERTY_HINT_RANGE, "0,1"), "set_volume", "get_volume");
